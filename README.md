...
국내 주요 기업 **100개**의 공식 채용 경로를 출발점으로 삼고, **사람인·잡코리아·원티드**에서 탐색을 보완합니다. 관심 가는 공고는 공식 원문에서 자격요건과 마감을 확인합니다. 접근이 막힌 사이트를 ‘채용 없음’으로 처리하지 않습니다.
...
| 도구 검증 | 테스트 243개 통과. Python 3.11·3.13 및 별도 설치 패키지에서 실행, 독립 품질 재검토 통과 |

숫자는 실행 기록이며 합격률이나 채용시장 전체 수집률이 아닙니다. [검증 기록과 재현 근거](artifacts/VERIFICATION.md)를 공개합니다.

**지원 범위도 분명히 합니다.** 기업 100개는 매출 순위가 아닌 채용용 선정 목록입니다. 검증 당시 잡코리아는 브라우저 확인이 필요했고, 원티드·카카오·롯데 직접 수집은 robots 정책 확인 실패로 중단됐습니다. 허용된 검색·브라우저 또는 사용자가 제공한 원문으로 보완하며, 모든 사이트의 완전 자동 수집을 보장하지 않습니다. 에이전트 연결 파일은 검사했지만 다섯 에이전트 각각의 실제 모델 작업까지 시험한 것은 아닙니다.
