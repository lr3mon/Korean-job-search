...
Windows PowerShell에서는 `py -3 -m korean_job_search setup`과 `py -3 -m korean_job_search init`을 사용하세요. Windows Python 3.10·3.13 CI에서 설치와 Chromium 실행을 [확인했습니다](https://github.com/lr3mon/Korean-job-search/actions/runs/35811489106). `setup`은 관리자 권한이나 시스템 패키지를 설치하지 않습니다. Linux에서 브라우저 실행에 필요한 OS 라이브러리가 없으면 별도로 준비해야 합니다.
