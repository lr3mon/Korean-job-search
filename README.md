...
Python 3.10+가 있는 환경에서 저장소 폴더를 열고 실행하세요. 기본 CLI에는 별도 라이브러리가 필요하지 않습니다.

```bash
python3 -m korean_job_search init
```

`workspace/`에 빈 프로필과 경험 서식이 만들어집니다. 여기에 이력서와 경력 자료를 두세요. 개인 자료는 기본적으로 Git 추적에서 제외됩니다.
...
workspace의 제 경력 자료를 바탕으로 AI 서비스 기획·사업개발 공고를 찾아주세요.
...
> **내 경험은 내 작업실에.** 개인정보를 공용 스킬이나 에이전트 지침에 넣지 않습니다. 단, 클라우드 모델을 쓰는 에이전트가 자료를 읽으면 해당 제공자에게 전송될 수 있습니다. [개인정보 안내](SECURITY.md)

## 실제 공고로 확인했습니다

| 확인한 것 | 실행 결과 |
| :--- | :--- |
| 공식 채용 경로 | 주요 기업 100개, 출처 근거 258개. 기업 기록 96개 검증·4개 부분 검증 |
| 실제 공고 처리 | NAVER·CJ·사람인 목록 15개 수집, NAVER 기획 직무 JD 확인 후 작성용 자료 묶음과 HTML 보고서 생성 |
| 기본 도구 검증 | 초기 테스트 243개 통과. Python 3.11·3.13 및 별도 설치 패키지에서 실행, 독립 품질 재검토 통과 |
| Scrapling 보조 도구 | 실제 목록 추출 → 상세 iframe JD → 기존 가져오기 흐름 확인. [추가 검증 기록](artifacts/SCRAPLING_VERIFICATION.md) |

숫자는 실행 기록이며 합격률이나 채용시장 전체 수집률이 아닙니다. [검증 기록과 재현 근거](artifacts/VERIFICATION.md)를 공개합니다.

**지원 범위도 분명히 합니다.** 기업 100개는 매출 순위가 아닌 채용용 선정 목록입니다. 잡코리아는 기존 파서가 놓치던 공개 HTML과 상세 iframe을 Scrapling 보조 도구로 읽을 수 있습니다. 원티드는 공개 페이지 열람과 robots 정책 확인을 구분하며, 정책 미확인 상태의 자동 수집은 중단합니다. 카카오·롯데도 초기 직접 수집에서 robots 확인 실패가 있었으며, 모든 사이트의 완전 자동 수집을 보장하지 않습니다. 에이전트 연결 파일은 검사했지만 다섯 에이전트 각각의 실제 모델 작업까지 시험한 것은 아닙니다.
