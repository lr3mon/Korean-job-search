...
수정할 워크플로우 원본은 `.agents/skills/korean-job-search/` 하나다. `SKILL.md`, 참조 문서, 빈 양식, 선택적 Scrapling 보조 스크립트를 함께 제공한다. 에이전트별 별도 프롬프트나 수집 구현은 없다. Scrapling MCP 제공 여부는 호스트마다 다르므로 현재 스키마를 확인하고, 없으면 공통 Python 스크립트를 격리 환경에서 실행한다. 설치·신뢰 설정은 자동 변경하지 않는다.
