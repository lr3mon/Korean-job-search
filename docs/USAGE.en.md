...
Use `py -3` (or an installed `python`) on Windows, for example `py -3 -m korean_job_search setup`. `setup` installs Scrapling in the project-local `workspace/tools/scrapling/` virtual environment, downloads browser files into Scrapling's default user cache, and launches Chromium to check readiness. Run it once, not before every command. It does not request administrator privileges or install system packages; missing Linux OS libraries are a separate prerequisite. `python -m pip install -e .` installs the Scrapling Python package and `kjs` command, but not the browser binary. Skill-only installers copy files without installing either dependency. `init` creates a blank private profile and story templates in `workspace/`; no fabricated candidate data is supplied. Leave your resume and evidence in their existing folder, give the agent that folder's path, and have it read only relevant files to fill the working profile and drafts in `workspace/`.
