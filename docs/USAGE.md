...
[프로젝트 소개로 돌아가기](../README.md) · [English](USAGE.en.md)

명령과 파일 경로는 모두 저장소 루트를 기준으로 합니다. 처음에는 필요한 단계만 실행하세요.
...
Windows에서는 `python3` 대신 `py -3`(또는 설치된 `python`)을 사용하세요. 예: `py -3 -m korean_job_search setup`. `setup`은 네트워크에서 Scrapling을 내려받아 `workspace/tools/scrapling/` 가상환경을 준비하고 브라우저 파일을 Scrapling의 기본 사용자 캐시에 다운로드한 뒤 실행을 확인합니다. 한 번 설치한 뒤에는 매번 실행할 필요가 없습니다. 시스템 패키지나 관리자 권한을 자동으로 사용하지 않습니다. Linux에서 브라우저 실행에 필요한 OS 라이브러리가 없으면 별도로 준비해야 합니다. `python -m pip install -e .`은 Scrapling 라이브러리와 `kjs` 명령을 설치하지만 브라우저 바이너리는 설치하지 않습니다. 스킬 파일만 복사하는 설치로는 패키지·브라우저가 설치되지 않습니다.
...
설정/호출 방식과 버전별 차이는 [호환성 문서](AGENTS_COMPATIBILITY.md)에 있습니다. 형식 호환, 파일 발견, 실제 모델로 수행한 결과는 서로 다른 검증입니다. 이 레포가 모든 런타임의 인증이나 검색 API를 제공하는 것은 아닙니다.
...
macOS에서 시스템 임시 폴더의 심볼릭 링크 때문에 경로 검증 테스트가 거부되면, 저장소 안의 실제 경로를 임시 폴더로 지정하세요. 안전 검사를 끄지 않아도 됩니다.
