...
|---|---|---|
| Generic careers pages | Schema.org `JobPosting` in JSON-LD, including `@graph` / `ItemList` | HTML/JSON-LD only. Arbitrary anchors and search-engine snippets are not postings. A 200 application shell is `needs_browser`, not `empty`. |
| NAVER Careers | Server-rendered `card_item` fallback; observed public GET `/rcrt/loadJobList.do` including `firstIndex=0`; detail `.detail_wrap` text | Employers come from `sysCompanyCdNm` in the public API or explicit detail fields. The HTML template's blind logo text incorrectly says NAVER for affiliates and is **not** employer evidence; HTML-only fallback uses `Unknown employer`. At most 3 API pages after discovery. `totalRows` is reconciled. HTML card dates remain date-only; API/detail times remain explicit KST. |
