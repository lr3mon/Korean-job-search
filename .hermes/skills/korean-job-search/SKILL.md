...
license: MIT
compatibility: "Python 3.10+ standard library CLI in repository root; optional host web/browser tools; no built-in LLM."
---
...
- 실제 이력·증빙·자기소개서·면접·접수 기록은 Git에서 제외된 **`workspace/` 안에만** 저장한다. `AGENTS.md`, 스킬, 테스트, 공개 보고서에 지원자 프로필을 넣지 않는다.
...
사용자에게 이미 받은 파일·희망 직무·경력 구분·근무지·배제 조건을 먼저 확인한다. 접근 허용을 받지 않은 개인 폴더나 다른 에이전트의 메모리를 뒤지지 않는다. 빠진 의사결정만 묻는다. `workspace/profile.json`의 조건과 경험을 실제 자료로 채우고 [경험 장부](templates/story-evidence.md)를 사용해 증거를 분리한다.
